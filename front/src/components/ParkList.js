...
function displayTrickList(trickList) {
    ...
    let c = 
        <React.Fragment>
            {
                platforms.map(pname => (
                    <div key={pname} className='parklist-platform box message is-info'>
                        <h3 key={pname + "-h3"} className='subtitle is-3 platform-name message-header'>{pname}</h3>
                        <ul key={pname + "-ul"} className='trick-bag'>
                            {
                                trickList.filter(t => t.platform.name === pname).map(t => 
                                    <li key={pname + " " + t.name}>
...
function fetchTrickList(tListSetter) {
    ...
    axios.get(Settings.getApiUrl('/tricks/') + currentUserId)
    .then(function(res) {
